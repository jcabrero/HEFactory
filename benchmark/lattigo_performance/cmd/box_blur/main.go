...
func main() {
	...
	// Argument parsing
	var input_params_file = flag.String("ip", "bin/data/box_blur.input.params", "A file to input parameters from users for prime generation")
	...
	row_sec := []string{"box_blur", strconv.Itoa(*precision), strconv.Itoa(*performance), strconv.Itoa(*security), strconv.Itoa(*test_num), strconv.Itoa(int(params.LogN())), strconv.Itoa(int(params.LogSlots())), strconv.Itoa(int(params.LogQP())), strconv.Itoa(int(params.Levels())), strconv.Itoa(int(math.Log2(params.Scale())))}
	...
	for i := range filter {
		...
		for j := range filter[i] {
			filter[i][j] = complex(float64(1 / (f_x * f_y)), 0)
		}
	}
	...
	row := []string{"box_blur", strconv.Itoa(*test_num), strconv.Itoa(a), strconv.Itoa(b), strconv.Itoa(c), strconv.Itoa(d), strconv.Itoa(e)}
