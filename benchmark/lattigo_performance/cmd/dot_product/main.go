...
func main() {
	...
	plaintext_vals := make([]complex128, n)
	for i := range plaintext_vals {
		plaintext_vals[i] = complex(float64(n - i), 0)
	}
	...
	plaintext_vals_pt := encoder.EncodeNTTAtLvlNew(params.MaxLevel(), plaintext_vals, params.LogSlots())

	result = evaluator.MulRelinNew(ciphertext, plaintext_vals_pt, rlk)
