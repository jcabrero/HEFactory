...
func main() {
	...
	// Argument parsing
	var input_params_file = flag.String("ip", "bin/data/l2_distance.input.params", "A file to input parameters from users for prime generation")
	...
	row_sec := []string{"l2_distance", strconv.Itoa(*precision), strconv.Itoa(*performance), strconv.Itoa(*security), strconv.Itoa(*test_num), strconv.Itoa(int(params.LogN())), strconv.Itoa(int(params.LogSlots())), strconv.Itoa(int(params.LogQP())), strconv.Itoa(int(params.Levels())), strconv.Itoa(int(math.Log2(params.Scale())))}
	...
	for i := range values_a {
		values_a[i] = complex(float64(rand.Intn(8)), 0)
	}

	values_b := make([]complex128, n)
	for i := range values_b {
		values_b[i] = complex(float64(rand.Intn(8)), 0)
	}
	...
	var plaintext_a, plaintext_b *ckks.Plaintext
	...
	ciphertext_a := ciphertext_vector(n)
	ciphertext_b := ciphertext_vector(n)
	var result *ckks.Ciphertext =  &ckks.Ciphertext{Element:&ckks.Element{}}
	
	for i := range ciphertext_a {
		plaintext_a = encoder.EncodeNew(values_a[i:i], 1)
		plaintext_b = encoder.EncodeNew(values_b[i:i], 1)
		ciphertext_a[i] = encryptor.EncryptNew(plaintext_a)	
		ciphertext_b[i] = encryptor.EncryptNew(plaintext_b)
	}
	...
	for i := range ciphertext_a{
		neg_ciphertext_b := evaluator.NegNew(ciphertext_b[i])
		t1 := evaluator.AddNew(ciphertext_a[i], neg_ciphertext_b)
		t2 := evaluator.MulRelinNew(t1, t1, rlk)
		if err := evaluator.Rescale(t2, params.Scale(), t2); err != nil {
			log.Fatal("Could not rescale the ciphertext")
		}
		if i == 0 {
			result = t2
		} else {
			result = evaluator.AddNew(result, t2)
		}
	...
	for i := 0; i < 2; i++{
		...
		t = evaluator.NegNew(t)
		t = evaluator.AddConstNew(result, complex(1, 0))
		result = evaluator.MulRelinNew(result, t, rlk)
		...
		// (b * b) * ((b - 3) * 0.5)
		if i != 1 {
			t = evaluator.AddConstNew(B, complex(-3, 0))
			t = evaluator.MultByConstNew(B, complex(0.25, 0))
			if err := evaluator.Rescale(t, params.Scale(), t); err != nil {
				log.Fatal("Could not rescale the ciphertext")
			}
			u := evaluator.MulRelinNew(B, B, rlk)
			if err := evaluator.Rescale(u, params.Scale(), u); err != nil {
				log.Fatal("Could not rescale the ciphertext")
			}
	
			B = evaluator.MulRelinNew(t, u, rlk)
	...
	row := []string{"l2_distance", strconv.Itoa(*test_num), strconv.Itoa(a), strconv.Itoa(b), strconv.Itoa(c), strconv.Itoa(d), strconv.Itoa(e)}
