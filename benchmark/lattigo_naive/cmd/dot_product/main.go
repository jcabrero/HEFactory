...
func main() {
	...
	// Argument parsing
	var input_params_file = flag.String("ip", "bin/data/dot_product.input.params", "A file to input parameters from users for prime generation")
	...
	row_sec := []string{"dot_product", strconv.Itoa(*precision), strconv.Itoa(*performance), strconv.Itoa(*security), strconv.Itoa(*test_num), strconv.Itoa(int(params.LogN())), strconv.Itoa(int(params.LogSlots())), strconv.Itoa(int(params.LogQP())), strconv.Itoa(int(params.Levels())), strconv.Itoa(int(math.Log2(params.Scale())))}
	...
	for i := range ciphertext_a {
		plaintext = encoder.EncodeNew(values[i:i], 1)
		ciphertext_a[i] = encryptor.EncryptNew(plaintext)	
		ciphertext_b[i] = encryptor.EncryptNew(plaintext)
	}
	...
	for i := 0; i < n; i++ {
		t := evaluator.MulRelinNew(ciphertext_a[i], ciphertext_b[i], rlk)
	...
	row := []string{"dot_product", strconv.Itoa(*test_num), strconv.Itoa(a), strconv.Itoa(b), strconv.Itoa(c), strconv.Itoa(d), strconv.Itoa(e)}
