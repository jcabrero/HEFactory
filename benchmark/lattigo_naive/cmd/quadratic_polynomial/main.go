...
func main() {
	...
	// Argument parsing
	var input_params_file = flag.String("ip", "bin/data/quadratic_polynomial.input.params", "A file to input parameters from users for prime generation")
	var ckks_params_file = flag.String("p", "data/quadratic_polynomial..ckks.params", "A file for storing the CKKS Parameters")
	...
	row_sec := []string{"quadratic_polynomial", strconv.Itoa(*precision), strconv.Itoa(*performance), strconv.Itoa(*security), strconv.Itoa(*test_num), strconv.Itoa(int(params.LogN())), strconv.Itoa(int(params.LogSlots())), strconv.Itoa(int(params.LogQP())), strconv.Itoa(int(params.Levels())), strconv.Itoa(int(math.Log2(params.Scale())))}
	...
	ciphertext := ciphertext_vector(n)
	result := ciphertext_vector(n)
	...
	// Polynomial : 2x^2 + 3x + 7

	for i := range ciphertext {
		x_2 := evaluator.MulRelinNew(ciphertext[i], ciphertext[i], rlk)
		...
		xt3 := evaluator.MultByConstNew(ciphertext[i], complex(3.0, 0))
		...
		result[i] = evaluator.AddConstNew(x_2t2pxt3, complex(7, 0))
	}
	...
	var decrypted [][]complex128 = make([][]complex128, n)
	...
	row := []string{"quadratic_polynomial", strconv.Itoa(*test_num), strconv.Itoa(a), strconv.Itoa(b), strconv.Itoa(c), strconv.Itoa(d), strconv.Itoa(e)}
